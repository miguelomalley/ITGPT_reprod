...
class OnsetModel(nn.Module):
    def __init__(self, cfg: OnsetConfig):
        ...
        self.frame_compress = GatedCompression(24 * d, d)
    ...
    def forward(self, x: torch.Tensor, bpm, difficulty, target=None):
        ...
        h = h.reshape(B * T, -1)
        h = self.frame_compress(h)  # Out: (B * T, d)

        # 6. Level 6: Global Chart Architecture over uniform full beat embeddings
        # Reconstruct the sequence dimensions: (B * T, d) -> (B, T, d)
        h_global = h.view(B, T, -1)
        h_global = h_global + self.level6_pos[:, :T, :]
...
def tversky_loss(pred_logits, targets, alpha=0.8, beta=0.2, eps=1e-5):
    # Convert logits to probabilities per lane
    probs = torch.sigmoid(pred_logits) # (B, T, 48)
    
    # Calculate true positives, false positives, false negatives per beat
    ...
    # Compute Tversky index per beat
    tversky_score = (true_pos + eps) / (true_pos + alpha * false_pos + beta * false_neg + eps)
...
def train_onset(
    ...
    best_ckpt = os.path.join(model_dir, f"{model_name}_nohier_best.pt")
    best_f1_ckpt = os.path.join(model_dir, f"{model_name}_nohier_best_f1.pt")
    start_epoch = 1
    best_score = -1.0
    best_f1 = -1.0
    ...
    for epoch in range(start_epoch, nepochs + 1):
        ...
        for i in pbar:
            ...
            #dice_loss = tversky_loss(logits, target)
            ...
            diag_input = hard + probs - probs.detach()
